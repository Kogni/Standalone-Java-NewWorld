...
public class GUI_Main2 extends JFrame implements ActionListener {
	...
	JButton Knapp;
	JProgressBar ExpeditionBar;
	JProgressBar BuildingBar;
	JProgressBar ResourceBar;
	JProgressBar MonthBar;
	...
	JLabel[] ResurssTekster = new JLabel[99];
	JLabel[] ResurssProduksjon = new JLabel[99];
	JLabel[] LandTekster = new JLabel[13];
	JLabel ExpeditionTekst;
	JLabel PopulationTekst;
	JLabel PopulationSizeTekst;
	JButton[] BygningsKnapper = new JButton[99];
	
	int Bygningsantall;
	
	private Brain_Main myControl;
	...
	public void setControl(Brain_Main Brain_Main){
		myControl = Brain_Main;
	}
	
	public void SettOpp(Object_Resource Array_Resources2[], Object_Landtypes Array_Land2[], Object_BuildingType Array_ByggTyper2[], Object_Building Array_Bygg2[], int Bygningsantallet){
	    	...
	    	for ( int x = 0 ; x < Bygningsantall+1 ; x++ ) {
		    	if (Array_Bygg[x].HentNavn() == BygningsNavn.getText()){
	    			...
	    			if ( Array_ByggTyper[x].BuildSpot.LandStorage < (Array_Bygg[x].Level+1) ){
	    				...
	    				//System.out.println("Enough land, have " + Array_ByggTyper[x].BuildSpot.LandStorage + " and need only " + (Array_Bygg[x].Level+1));
	    			}
